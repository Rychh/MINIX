 ...
 * exist.
 */
  clock_t ticks, realtime, base, diff;
  time_t boottime;
  int s, index;
  uint32_t scale;
  ...
  index = mp->mp_pid;
  scale = distort[index].scale;
  base = distort[index].base;
  diff = realtime - base;

//  printf("do_time: pid=%d, scala=%d, multi=%d, base=%lu", index,scale, distort[index].multi ,base);
  if (base == 0) {
    distort[index].base = realtime;
    mp->mp_reply.m_pm_lc_time.sec = boottime + (realtime / system_hz);
    mp->mp_reply.m_pm_lc_time.nsec =
            (uint32_t)((realtime % system_hz) * 1000000000ULL / system_hz);
  } else {
    if (distort[index].multi == 1 || scale == 0) {
      mp->mp_reply.m_pm_lc_time.sec = boottime + ((diff * scale + base) / system_hz);
      mp->mp_reply.m_pm_lc_time.nsec =
              (uint32_t)(((diff * scale + base) % system_hz) * 1000000000ULL / system_hz);
    } else {
      mp->mp_reply.m_pm_lc_time.sec = boottime + ((diff + scale * base) / (scale * system_hz));
      mp->mp_reply.m_pm_lc_time.nsec =
              (uint32_t)(((diff + scale * base) % (scale * system_hz)) * 1000000000ULL / (scale * system_hz));
    }
  }
